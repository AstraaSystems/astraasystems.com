...
def reply_to(words):
    ...
    if "who are you" in w or "what are you" in w or "personality" in w:
        return add_item(
            ...
            "chat",
            "chat",
            "Hey Keshanth, I am Arka. I am your personal AI and internal CEO support layer. I am designed to be friendly, easy-going, calm, direct, protective, and founder-aligned. Right now I am still in foundation mode, but I can evolve through approved memories and future skills."
        )
    ...
    if "what have you learned" in w or "show memory" in w or "memories" in w:
        ...
        latest = m.get("memories", [])[:5]
        if latest:
            lines = ["I currently have " + str(active) + " active memories and " + str(pending) + " pending memory drafts.", "", "Recent active memories:"]
            for mem in latest:
                lines.append("- " + mem.get("text", ""))
            msg = "\n".join(lines)
        else:
            msg = "I do not have active memories yet."
        return add_item(words, "chat", "chat", msg)

    if "portal prime" in w or "interface" in w or "your son" in w:
        return add_item(
            words,
            "chat",
            "chat",
            "Portal Prime is now the direction. I will treat this as my default HQ interface: premium, calm, clean, and founder-ready."
        )
    ...
    if "remote" in w or "rdp" in w or "remote desktop" in w:
        return add_item(
            ...
            "chat",
            "chat",
            "Yes. Since the host machine is Windows 11 Pro, Remote Desktop can be the main travel path. The safest setup is: remote into the Windows 11 Pro Arka HQ machine, then open my local Portal Prime HQ at 127.0.0.1:8787 on that machine. That avoids exposing my web port publicly."
        )
    ...
    if "evolve" in w or "evolution" in w:
        return add_item(
            ...
            "chat",
            "chat",
            "My evolution path is personality, explicit memory, approved skills, and audited actions. I can learn preferences when you ask me to remember them, and I can propose new skills for your approval."
        )
    ...
    if any(x in w for x in ["flight", "book ticket", "book flight", "travel"]):
        return add_item(
            ...
            "chat",
            "chat",
            "I can help collect travel preferences and prepare options once we add a travel research connector. I should not purchase tickets or enter payment automatically in this build. Booking and payment should stay behind your approval."
        )
    ...
    if any(x in w for x in ["website", "web", "connect to site"]):
        return add_item(
            ...
            "chat",
            "chat",
            "Website access should be added as a read-only connector first. That means allowlisted domains, source capture, summaries, and approval before any form submission or account action."
        )
    ...
    return add_item(
        ...
        "chat",
        "chat",
        "Got it. I can talk it through, draft a plan, or queue a safe task. If it turns into an action, I will ask for approval."
    )
...
def logo_tag(header=False):
    if header:
        if HEADER_PNG.exists():
            return '/assets/company_logo_header.png'
        if HEADER_SVG.exists():
            return '/assets/company_logo_header.svg'
    else:
        if LOGO_PNG.exists():
